=== SIMULATION LOGS ===

--- Encounter Start: Players vs Monsters ---

=== Pre-Combat Setup ===
  > Kaelen uses Rage
      -> Casts Rage on Kaelen
  > Boris uses Bless
    - Applying template: Bless
      Template Bless applied to target


# Round 1

## Erica (HP: 22/22)
    - Uses Action: Bane
    - Applying template: Bane
      Template Bane applied to target

## Boris (HP: 22/22)
    - Uses Action: Dodge
      -> Casts Dodge on Boris

## Alestair (HP: 54/54)
    - Uses Action: Divine Smite
      -> Casts Divine Smite on Alestair
    - Uses Action: Greatsword x2
* ⚔️ Attack vs **Erica**: **12** vs AC 18 -> ❌ **MISS**
* ⚔️ Attack vs **Erica**: **21** vs AC 18 -> ✅ **HIT**
  * 🩸 Damage: **34**
  * 💀 **Erica falls unconscious!**

## Van (HP: 57/57)
    - Uses Action: Action Surge: Greatsword x2
* ⚔️ Attack vs **Boris**: **15** vs AC 18 -> ❌ **MISS**
* ⚔️ Attack vs **Boris**: **25** vs AC 18 -> ✅ **HIT**
  * 🩸 Damage: **10**
    - Uses Action: Greatsword x2
* ⚔️ Attack vs **Boris**: **21** vs AC 18 -> ✅ **HIT**
  * 🩸 Damage: **14**
  * 💀 **Boris falls unconscious!**
      -> Boris is already unconscious, skipping attack

## Kaelen (HP: 90/90)
    - Uses Action: Death Damage
* ⚔️ Attack vs **Bob**: **101** vs AC 14 (MISS!) -> ❌ **MISS**
    - Uses Action: Reckless Attack
      -> Casts Reckless Attack on Kaelen
    - Uses Action: Greatsword x2
* ⚔️ Attack vs **Bob**: **16** vs AC 14 -> ✅ **HIT**
  * 🩸 Damage: **17**
* ⚔️ Attack vs **Bob**: **20** vs AC 14 -> ✅ **HIT**
  * 🩸 Damage: **12**
  * 💀 **Bob falls unconscious!**
    - Uses Action: Longtooth
* ⚔️ Attack vs **Alestair**: **27** vs AC 20 -> ✅ **HIT**
  * 🩸 Damage: **11**

## Andreas (HP: 54/54)
    - Uses Action: Hunter's Mark
    - Applying template: Hunter's Mark
      Template Hunter's Mark applied to target
    - Uses Action: Crossbow Expert
* ⚔️ Attack vs **Kaelen**: **9** vs AC 15 -> ❌ **MISS**

# Round 2

## Alestair (HP: 43/54)
    - Uses Action: Lay on Hands
      -> Heals Bob for 35 HP (was at -15/20)

## Van (HP: 57/57)
    - Uses Action: Greatsword x2
* ⚔️ Attack vs **Kaelen**: **22** vs AC 15 -> ✅ **HIT**
  * 🩸 Damage: **7** * 0.50 ()
* ⚔️ Attack vs **Kaelen**: **16** vs AC 15 -> ✅ **HIT**
  * 🩸 Damage: **7** * 0.50 ()

## Kaelen (HP: 76/90)
    - Uses Action: Reckless Attack
      -> Casts Reckless Attack on Kaelen
    - Uses Action: Greatsword x2
* ⚔️ Attack vs **Bob**: **29** vs AC 14 (CRIT!) -> ✅ **HIT**
  * 🩸 Damage: **22**
  * 💀 **Bob falls unconscious!**
      -> Bob is already unconscious, skipping attack
    - Uses Action: Longtooth
* ⚔️ Attack vs **Alestair**: **23** vs AC 20 -> ✅ **HIT**
  * 🩸 Damage: **9**

## Andreas (HP: 54/54)
    - Uses Action: Hunter's Mark
    - Applying template: Hunter's Mark
      Template Hunter's Mark applied to target
    - Uses Action: Crossbow Expert
* ⚔️ Attack vs **Kaelen**: **16** vs AC 15 -> ✅ **HIT**
  * 🩸 Damage: **9** * 0.50 ()

# Round 3

## Alestair (HP: 34/54)
    - Uses Action: Greatsword x2
* ⚔️ Attack vs **Kaelen**: **16** vs AC 15 -> ✅ **HIT**
  * 🩸 Damage: **12** * 0.50 ()
* ⚔️ Attack vs **Kaelen**: **21** vs AC 15 -> ✅ **HIT**
  * 🩸 Damage: **13** * 0.50 ()

## Van (HP: 57/57)
    - Uses Action: Greatsword x2
* ⚔️ Attack vs **Kaelen**: **27** vs AC 15 -> ✅ **HIT**
  * 🩸 Damage: **7** * 0.50 ()
* ⚔️ Attack vs **Kaelen**: **17** vs AC 15 -> ✅ **HIT**
  * 🩸 Damage: **7** * 0.50 ()

## Kaelen (HP: 28/90)
    - Uses Action: Reckless Attack
      -> Casts Reckless Attack on Kaelen
    - Uses Action: Greatsword x2
* ⚔️ Attack vs **Alestair**: **24** vs AC 20 -> ✅ **HIT**
  * 🩸 Damage: **19**
* ⚔️ Attack vs **Alestair**: **21** vs AC 20 -> ✅ **HIT**
  * 🩸 Damage: **14**
    - Uses Action: Longtooth
* ⚔️ Attack vs **Alestair**: **28** vs AC 20 -> ✅ **HIT**
  * 🩸 Damage: **7**
  * 💀 **Alestair falls unconscious!**

## Andreas (HP: 54/54)
    - Uses Action: Hunter's Mark
    - Applying template: Hunter's Mark
      Template Hunter's Mark applied to target
    - Uses Action: Crossbow Expert
* ⚔️ Attack vs **Kaelen**: **13** vs AC 15 -> ❌ **MISS**

# Round 4

## Van (HP: 57/57)
    - Uses Action: Greatsword x2
* ⚔️ Attack vs **Kaelen**: **15** vs AC 15 -> ✅ **HIT**
  * 🩸 Damage: **6** * 0.50 ()
* ⚔️ Attack vs **Kaelen**: **11** vs AC 15 (MISS!) -> ❌ **MISS**

## Kaelen (HP: 22/90)
    - Uses Action: Reckless Attack
      -> Casts Reckless Attack on Kaelen
    - Uses Action: Greatsword x2
* ⚔️ Attack vs **Andreas**: **26** vs AC 18 -> ✅ **HIT**
  * 🩸 Damage: **17**
* ⚔️ Attack vs **Andreas**: **13** vs AC 18 -> ❌ **MISS**
    - Uses Action: Longtooth
* ⚔️ Attack vs **Andreas**: **20** vs AC 18 -> ✅ **HIT**
  * 🩸 Damage: **11**

## Andreas (HP: 26/54)
    - Uses Action: Hunter's Mark
    - Applying template: Hunter's Mark
      Template Hunter's Mark applied to target
    - Uses Action: Crossbow Expert
* ⚔️ Attack vs **Kaelen**: **23** vs AC 15 -> ✅ **HIT**
  * 🩸 Damage: **8** * 0.50 ()

# Round 5

## Van (HP: 57/57)
    - Uses Action: Greatsword x2
* ⚔️ Attack vs **Kaelen**: **13** vs AC 15 -> ❌ **MISS**
* ⚔️ Attack vs **Kaelen**: **18** vs AC 15 -> ✅ **HIT**
  * 🩸 Damage: **8** * 0.50 ()

## Kaelen (HP: 6/90)
    - Uses Action: Reckless Attack
      -> Casts Reckless Attack on Kaelen
    - Uses Action: Greatsword x2
* ⚔️ Attack vs **Andreas**: **27** vs AC 18 -> ✅ **HIT**
  * 🩸 Damage: **12**
* ⚔️ Attack vs **Andreas**: **29** vs AC 18 (CRIT!) -> ✅ **HIT**
  * 🩸 Damage: **25**
  * 💀 **Andreas falls unconscious!**
    - Uses Action: Longtooth
* ⚔️ Attack vs **Van**: **16** vs AC 20 -> ❌ **MISS**

# Round 6

## Van (HP: 57/57)
    - Uses Action: Greatsword x2
* ⚔️ Attack vs **Kaelen**: **28** vs AC 15 -> ✅ **HIT**
  * 🩸 Damage: **8** * 0.50 ()
  * 💀 **Kaelen falls unconscious!**
      -> Kaelen is already unconscious, skipping attack

=== RESULTS ===

Encounter 1: 6 rounds
