...
fn main() {
    ...
    let content = fs::read_to_string("tests/scenarios/andrew_vs_bernd_a_wins.json")
    ...
    assert_eq!(results.len(), iterations, "Should have {} results", iterations);
    ...
    for (i, result) in results.iter().enumerate() {
        ...
        if score < prev_score {
            println!("❌ Result {} is out of order: score {} < prev {}", i, score, prev_score);
            sorted = false;
            break;
        }
        prev_score = score;
