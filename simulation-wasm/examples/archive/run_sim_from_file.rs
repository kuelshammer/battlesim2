...
fn main() {
    ...
    let input: SimulationInput =
        serde_json::from_str(&input_content).expect("Failed to parse JSON");

    println!(
        "Running {} simulation with {} players and {} encounters...",
        if use_event_driven {
            "event-driven"
        } else {
            "legacy"
    ...
    let (results, logs): (_, Vec<String>) = if use_event_driven {
        ...
        let string_logs = events
            .into_iter()
            .map(|event| serde_json::to_string(&event).unwrap_or_default())
            .collect();
        (res, string_logs)
    } else {
        let (results, events) = run_event_driven_simulation_rust(input.players, input.encounters, 1, true);
        let string_logs = events
            .into_iter()
            .map(|event| serde_json::to_string(&event).unwrap_or_default())
            .collect();
