...
fn test_fast_simulation_iterations() {
    ...
    assert_eq!(
        runs.len(),
        expected,
        "Should have exactly 100 runs (minimum enforced)"
    );
...
fn test_heavy_load_simulation() {
    ...
    for i in 1..=4 {
        players.push(Creature {
            ...
            initiative_advantage: false,
            actions: vec![simulation_wasm::model::Action::Atk(
                simulation_wasm::model::AtkAction {
                    id: "atk".to_string(),
...
fn test_extreme_load_simulation() {
    ...
    println!(
        "Extreme stress test: {} iterations, 26 combatants took {:?}",
        iterations, duration
    );
...
fn test_multi_encounter_stress() {
    ...
    println!(
        "Multi-encounter stress test: {} iterations took {:?}",
        iterations, duration
    );
}
...
fn stress_throughput_time_boxed() {
    ...
    assert!(
        count > 0,
        "No simulations completed in {}s - possible infinite loop!",
        time_secs
    );
}
...
fn stress_latency_count_boxed() {
    ...
    for _ in 0..target_sims {
        run_event_driven_simulation_rust(
            vec![player.clone()],
            timeline.clone(),
            iterations_per_sim,
            false,
