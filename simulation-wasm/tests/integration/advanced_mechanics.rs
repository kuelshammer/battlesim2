...
use simulation_wasm::resources::{ActionCost, ActionTag, ResourceType};
...
fn test_counterspell_interrupt_flow() {
    ...
    let mut counterspeller_state = CreatureState {
        current_hp: 100,
    ...
    assert!(
        result
            .events_generated
            .iter()
            .any(|e| matches!(e, Event::CastSpell { .. })),
        "CastSpell event should be emitted"
    );
    ...
    assert!(
        ...
        "Fireball should have been interrupted and dealt no damage"
    );
}
...
fn test_opportunity_attack_on_movement() {
    ...
    assert!(
        ...
        "UnitMoved event should be emitted in the event bus"
    );
...
fn test_bardic_inspiration_modification() {
    ...
    let mut initial_state = CreatureState {
        current_hp: 100,
        ..CreatureState::default()
    };
    ...
    assert!(
        hit_event.is_some(),
        "Attack should have hit after Bardic Inspiration"
    );
