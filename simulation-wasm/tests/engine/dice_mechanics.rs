...
use simulation_wasm::model::{
    ActionCondition, AtkAction, Combattant, Creature, CreatureState, Frequency,
};
...
fn test_critical_hit_logic() {
    ...
    assert!(
        events
            .iter()
            .any(|e| matches!(e, simulation_wasm::events::Event::AttackHit { .. })),
        "Crit should hit"
    );
    ...
    let dmg_event = events
        .iter()
        .find(|e| matches!(e, simulation_wasm::events::Event::DamageTaken { .. }));
    if let Some(simulation_wasm::events::Event::DamageTaken { damage, .. }) = dmg_event {
        assert_eq!(
            damage, &12.0,
            "Crit damage should only double dice (1d1->2d1), not constant (+10)"
        );
    }
    ...
    assert!(
        events
            .iter()
            .any(|e| matches!(e, simulation_wasm::events::Event::AttackMissed { .. })),
        "Natural 1 should miss"
    );
...
fn test_complex_to_hit_formula() {
    ...
    assert!(
        events
            .iter()
            .any(|e| matches!(e, simulation_wasm::events::Event::AttackHit { .. })),
        "Exact hit with formula should succeed"
    );
    ...
    assert!(
        events
            .iter()
            .any(|e| matches!(e, simulation_wasm::events::Event::AttackMissed { .. })),
        "Formula missing AC should fail"
    );
...
fn test_advantage_disadvantage_scenarios() {
    ...
    context.apply_effect(simulation_wasm::context::ActiveEffect {
        ...
        effect_type: simulation_wasm::context::EffectType::Condition(
            simulation_wasm::enums::CreatureCondition::AttacksWithDisadvantage,
        ),
    ...
    assert!(
        events
            .iter()
            .any(|e| matches!(e, simulation_wasm::events::Event::AttackMissed { .. })),
        "dis(20, 7) + 5 = 12 should miss AC 15"
    );
    ...
    context.apply_effect(simulation_wasm::context::ActiveEffect {
        ...
        target_id: "p1".to_string(),
        effect_type: simulation_wasm::context::EffectType::Condition(
            simulation_wasm::enums::CreatureCondition::AttacksWithAdvantage,
        ),
        remaining_duration: 10,
        conditions: vec![],
    });
    ...
    assert!(
        events
            .iter()
            .any(|e| matches!(e, simulation_wasm::events::Event::AttackHit { .. })),
        "adv(11, 20) should be a crit hit"
    );
    ...
    assert!(
        events
            .iter()
            .any(|e| matches!(e, simulation_wasm::events::Event::AttackMissed { .. })),
        "dis(17, 1) should miss (Natural 1)"
    );
    ...
    assert!(
        events
            .iter()
            .any(|e| matches!(e, simulation_wasm::events::Event::AttackHit { .. })),
        "adv(12, 1) + 5 = 17 should hit AC 15"
    );
...
fn test_triple_advantage_logic() {
    ...
    context.apply_effect(simulation_wasm::context::ActiveEffect {
        ...
        target_id: "p1".to_string(),
        effect_type: simulation_wasm::context::EffectType::Condition(
            simulation_wasm::enums::CreatureCondition::AttacksWithTripleAdvantage,
        ),
        remaining_duration: 10,
        conditions: vec![],
    });
    ...
    assert!(
        events
            .iter()
            .any(|e| matches!(e, simulation_wasm::events::Event::AttackHit { .. })),
        "Triple advantage should pick 20 from [1, 1, 20]"
    );
...
fn test_advantage_disadvantage_cancellation() {
    ...
    context.apply_effect(simulation_wasm::context::ActiveEffect {
        ...
        effect_type: simulation_wasm::context::EffectType::Condition(
            simulation_wasm::enums::CreatureCondition::IsAttackedWithDisadvantage,
        ),
        remaining_duration: 10,
        conditions: vec![],
    });
    ...
    assert!(
        events
            .iter()
            .any(|e| matches!(e, simulation_wasm::events::Event::AttackHit { .. })),
        "Cancellation should result in normal roll, picking first roll (20)"
    );
    ...
    assert!(
        events
            .iter()
            .any(|e| matches!(e, simulation_wasm::events::Event::AttackMissed { .. })),
        "Cancellation should result in normal roll, picking first roll (1)"
    );
