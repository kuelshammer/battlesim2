...
fn test_two_pass_reproducibility() {
    ...
    let survey_runs = run_survey_pass(
        players.clone(),
        timeline.clone(),
        iterations,
        Some(base_seed),
    ...
    assert_eq!(
        survey_runs.len(),
        expected,
        "Should have 100 survey runs (minimum enforced)"
    );
    ...
    for &index in &indices_to_verify {
        ...
        assert_eq!(
            lightweight_run.seed, expected_seed,
            "Seed mismatch at index {}",
            index
        );
        ...
        let full_score = full_result
            .score
            .expect("Full simulation should return a score");
        ...
        assert_eq!(
            ...
            "Survivor count mismatch at index {}",
            index
        );

        assert!(
            !full_result.encounters.is_empty(),
            "Full result should have encounters"
        );
    }
}
...
fn test_reproducibility_complex_mechanics() {
    ...
    for (index, lightweight_run) in survey_runs.iter().enumerate() {
        ...
        assert!(
            (lightweight_run.final_score - full_score).abs() < 1e-10,
