...
fn test_cumulative_log_sorting() {
    ...
    for i in 0..100 {
        ...
        events.push(Event::EncounterStarted {
            combatant_ids: vec!["p1".to_string()],
        });
    ...
    for log in &output1.decile_logs {
        assert!(log
            .iter()
            .any(|e| matches!(e, Event::Custom { event_type, .. } if event_type == "E1")));
        assert!(!log
            .iter()
            .any(|e| matches!(e, Event::Custom { event_type, .. } if event_type == "E2")));
    }
    ...
    for (i, run) in runs.iter_mut().enumerate().take(100) {
        ...
        run.result.encounters[1].rounds[0].team1[0]
            .final_state
            .current_hp = hp2;
    }
