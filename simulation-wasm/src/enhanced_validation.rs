use crate::error_handling::{ErrorContext, SimulationError};
use crate::model::*;
...
impl ValidationReport {
    ...
    pub fn is_acceptable(&self) -> bool {
        self.success_rate() >= 0.95 && self.error_count <= self.total_results / 20
        // Max 5% errors
    }
}

pub fn validate_simulation_results(
    results: &[SimulationResult],
) -> Result<ValidationReport, SimulationError> {
    let mut report = ValidationReport::new(results.len());

    // Check for empty results
    if results.is_empty() {
        return Err(SimulationError::EmptyResult(
            "No simulation results provided".to_string(),
        ));
    }
    ...
    if report.success_rate() < 0.95 {
        return Err(SimulationError::UnexpectedState(format!(
            "Success rate {:.1}% below 95% threshold",
...
fn validate_single_result(
    ...
    if result.is_empty() {
        return Err(SimulationError::EmptyResult(format!(
...
fn validate_round(
    ...
    let context =
        ErrorContext::new("validation".to_string(), iteration, encounter_idx).with_round(round_idx);

    // Check if both teams have combatants
    if round.team1.is_empty() && round.team2.is_empty() {
        return Err(SimulationError::EmptyResult(format!(
            "Round {} has no combatants",
            round_idx
        )));
    }
...
fn validate_combatant(
    ...
    if combatant.creature.hp == 0 {
        return Err(SimulationError::InvalidCombatant(format!(
            "Combatant '{}' has invalid base HP: {}",
            combatant.creature.name, combatant.creature.hp
        )));
    }
...
pub fn validate_scenario_for_edge_cases(
    players: &[Creature],
