...
pub fn check_action_condition(
    ...
    match &action.base().condition {
        ...
        ActionCondition::EnemyAdvantage
        | ActionCondition::PlayerAdvantage
        | ActionCondition::EvenActionEconomy => {
            let allies_refs: Vec<&Combattant> = allies.iter().collect();
            let enemies_refs: Vec<&Combattant> = enemies.iter().collect();
            let state =
                crate::strategic_assessment::calculate_action_economy(&allies_refs, &enemies_refs);
            match &action.base().condition {
                ActionCondition::EnemyAdvantage => {
                    state.state == crate::strategic_assessment::ActionEconomyState::EnemyAdvantage
                }
                ActionCondition::PlayerAdvantage => {
                    state.state == crate::strategic_assessment::ActionEconomyState::PlayerAdvantage
                }
                ActionCondition::EvenActionEconomy => {
                    state.state == crate::strategic_assessment::ActionEconomyState::Even
                }
                _ => unreachable!(),
            }
        }
...
pub fn get_actions(c: &Combattant, allies: &[Combattant], enemies: &[Combattant]) -> Vec<Action> {
    ...
    eprintln!(
        "      Getting actions for {}. Creature actions: {}",
        c.creature.name,
        c.creature.actions.len()
    );
    ...
    for action in &c.creature.actions {
        #[cfg(debug_assertions)]
        eprintln!(
            "        Considering action: {} (Slot: {:?}, Freq: {:?})",
            action.base().name,
            action.base().action_slot,
            action.base().freq
        );
        ...
        #[cfg(debug_assertions)]
        eprintln!(
            "          Action {} usable. Adding to result.",
            action.base().name
        );
...
pub fn is_usable(c: &Combattant, action: &Action) -> bool {
    #[cfg(debug_assertions)]
    eprintln!(
        "        Checking usability for {}: {}. Remaining uses: {:?}",
        c.creature.name,
        action.base().name,
        c.final_state.resources.current.get(&action.base().id)
    );
...
pub fn get_attack_roll_result(attacker: &Combattant) -> (f64, bool, bool) {
    ...
    let has_advantage = has_condition(attacker, CreatureCondition::AttacksWithAdvantage)
        || has_condition(
            attacker,
            CreatureCondition::AttacksAndIsAttackedWithAdvantage,
        );
    let has_disadvantage = has_condition(attacker, CreatureCondition::AttacksWithDisadvantage)
        || has_condition(attacker, CreatureCondition::AttacksAndSavesWithDisadvantage); // Assuming this also applies to attacks.
...
pub fn break_concentration(
    ...
    #[cfg(debug_assertions)]
    eprintln!(
        "        [DEBUG] break_concentration called: Caster ID: {}, Buff ID: {}",
        caster_id, buff_id
    );
...
pub fn remove_all_buffs_from_source(
    source_id: &str,
    allies: &mut [Combattant],
    enemies: &mut [Combattant],
) {
    #[cfg(debug_assertions)]
    eprintln!(
        "        [DEBUG] remove_all_buffs_from_source called: Source ID: {}",
        source_id
    );
    ...
    for c in allies.iter_mut().chain(enemies.iter_mut()) {
        if c.id == source_id {
            ...
            #[cfg(debug_assertions)]
            eprintln!(
                "          Cleared concentration and action targets on dead caster: {}",
                c.creature.name
            );
        }
    }
    ...
    for c in allies.iter_mut().chain(enemies.iter_mut()) {
        ...
        c.final_state.buffs.retain(|_buff_id, buff| {
            ...
            if !should_keep {
                #[cfg(debug_assertions)]
                eprintln!(
                    "          Removed buff '{}' from {} (source {} is dead)",
                    _buff_id, c.creature.name, source_id
                );
            }
            should_keep
        });
        ...
        if _before_count != _after_count {
            eprintln!(
                "          {} had {} buffs from dead source, now has {} buffs total",
                c.creature.name,
...
pub fn get_remaining_uses(
    creature: &Creature,
    rest: &str,
    old_value: Option<&HashMap<String, f64>>,
) -> HashMap<String, f64> {
    ...
    for action in &creature.actions {
        let val = match &action.base().freq {
            ...
            Frequency::Static(s) if s == "1/fight" => {
                if rest == "long rest" || rest == "short rest" {
                    1.0
                } else {
                    *old_value
                        .and_then(|m| m.get(&action.base().id))
                        .unwrap_or(&0.0)
                }
            }
