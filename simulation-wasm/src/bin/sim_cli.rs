...
fn run_aggregate(scenario_path: &PathBuf, output_path: Option<&std::path::Path>) {
    ...
    let quintile_labels = ["Worst 20%", "Below Average", "Median", "Above Average", "Best 20%"];
...
fn calculate_quintile_stats(results: &[SimulationResult], quintile_num: usize, label: &str) -> QuintileStats {
    let count = results.len() as f64;
    let mut wins = 0;
    let mut total_rounds = 0;
    
    // Track HP per combatant (id -> (name, total_hp, count))
    let mut combatant_hp: std::collections::HashMap<String, (String, f64, usize)> = std::collections::HashMap::new();
        ...
        if let Some(encounter) = result.last() {
            if let Some(last_round) = encounter.rounds.last() {
                ...
                for c in last_round.team1.iter().chain(last_round.team2.iter()) {
                    let entry = combatant_hp.entry(c.id.clone()).or_insert((c.creature.name.clone(), 0.0, 0));
...
fn run_log(scenario_path: &PathBuf, format: &str, run_index: Option<usize>) {
    ...
    match format {
        "json" => {
            ...
            let formatted_events: Vec<String> = events.iter()
...
fn print_markdown_log(result: &SimulationResult, events: &[String]) {
    ...
    if let Some(encounter) = result.first() {
        for (round_idx, round) in encounter.rounds.iter().enumerate() {
            ...
            all.sort_by(|a, b| b.initiative.partial_cmp(&a.initiative).unwrap_or(std::cmp::Ordering::Equal));
            ...
            for c in all {
                ...
                println!("- **HP**: {:.1} → {:.1}", c.initial_state.current_hp, c.final_state.current_hp);
                ...
                for action in &c.actions {
                    let targets: Vec<String> = action.targets.iter()
                        .map(|(id, count)| if *count > 1 { format!("{} (x{})", id, count) } else { id.clone() })
                        .collect();
...
fn get_run_metrics(result: &SimulationResult) -> (f64, usize) {
    if let Some(encounter) = result.last() {
        if let Some(last_round) = encounter.rounds.last() {
            ...
            let hps: Vec<f64> = winning_team.iter().map(|c| c.final_state.current_hp).collect();
...
fn run_breakdown(scenario_path: &PathBuf, run_index: Option<usize>) {
    ...
    let (results, events) = if let Some(idx) = run_index {
        let (res, events) = run_event_driven_simulation_rust(players.clone(), encounters.clone(), idx + 1, true);
    ...
    for (actor_id, actions) in breakdown {
        ...
        for (action_id, stats) in actions {
            ...
            if stats.hits + stats.misses > 0 {
                ...
                println!("    - Attacks: {} ({} Hits, {} Misses)", total_attacks, stats.hits, stats.misses);
                println!("    - Accuracy: {:.1}%", accuracy);
            }
            if stats.total_damage > 0.0 {
                println!("    - Total Damage: {:.1}", stats.total_damage);
                println!("    - Avg Dmg/Hit: {:.1}", stats.total_damage / stats.hits as f64);
                println!("    - Avg Dmg/Use: {:.1}", stats.total_damage / stats.uses as f64);
            }
            println!();
        }
        println!("--------------------------------------------------");
...
fn run_math(scenario_path: &PathBuf, attacker_name: &str, defender_name: &str) {
    ...
    let attacker = all_creatures.iter().find(|c| c.name == attacker_name || c.id == attacker_name);
    let defender = all_creatures.iter().find(|c| c.name == defender_name || c.id == defender_name);
    
    if attacker.is_none() || defender.is_none() {
        println!("Error: Could not find attacker '{}' or defender '{}'", attacker_name, defender_name);
    ...
    for action in &attacker.actions {
        if let Action::Atk(atk) = action {
            println!("Action: {}", if atk.name.is_empty() { "(Unnamed Attack)" } else { &atk.name });
            ...
            println!("  - Hit Chance: {:.1}% (Needs roll {:.0}+)", hit_chance * 100.0, needed_roll);
...
fn load_scenario(path: &PathBuf) -> (Vec<Creature>, Vec<Encounter>, String) {
    ...
    let players: Vec<Creature> = serde_json::from_value(data["players"].clone()).expect("Failed to parse players");
