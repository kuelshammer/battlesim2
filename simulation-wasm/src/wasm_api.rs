...
pub fn run_simulation_with_callback(
    ...
    for i in 0..iterations {
        ...
        let (result, _) = crate::run_single_event_driven_simulation(&players, &timeline, false);
    ...
    for (idx, &seed) in interesting_seeds.iter().enumerate() {
        crate::rng::seed_rng(seed);
        let (_, events) = crate::run_single_event_driven_simulation(&players, &timeline, true);
...
pub fn run_event_driven_simulation(players: JsValue, timeline: JsValue, iterations: usize) -> Result<JsValue, JsValue> {
    ...
    for i in 0..iterations {
        let (result, events) = crate::run_single_event_driven_simulation(&players, &timeline, i == 0);
        results.push(result);
