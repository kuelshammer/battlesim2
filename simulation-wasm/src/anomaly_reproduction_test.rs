...
mod tests {
    ...
    fn test_round_1_anomaly_reproduction() {
        ...
        assert!(
            !aggregated.is_empty(),
            "Aggregated rounds should not be empty"
        );

        let round1 = &aggregated[0];
        let aggregated_p1 = round1
            .team1
            .iter()
            .find(|c| c.creature.id == "p1")
            .expect("P1 not found");
        let aggregated_p2 = round1
            .team2
            .iter()
            .find(|c| c.creature.id == "p2")
            .expect("P2 not found");

        println!(
            "Round 1 Aggregated P1 HP: {}/{}",
            aggregated_p1.final_state.current_hp, aggregated_p1.creature.hp
        );
        println!(
            "Round 1 Aggregated P2 HP: {}/{}",
            aggregated_p2.final_state.current_hp, aggregated_p2.creature.hp
        );

        // P1 should have taken damage from P2 in Round 1
        assert!(
            aggregated_p1.final_state.current_hp < aggregated_p1.creature.hp,
            "P1 should have taken damage in Round 1, but HP is {}",
            aggregated_p1.final_state.current_hp
        );

        // P2 should also have taken damage from P1 in Round 1
        assert!(
            aggregated_p2.final_state.current_hp < aggregated_p2.creature.hp,
            "P2 should have taken damage in Round 1, but HP is {}",
            aggregated_p2.final_state.current_hp
        );
    }
}
