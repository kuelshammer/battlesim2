...
fn test_ledger_restoration() {
    ...
    ledger.register_resource(spell_slot.clone(), Some("1"), 4.0, Some(ResetType::LongRest));
...
fn test_ledger_reset() {
    ...
    ledger.register_resource(sr_resource.clone(), Some("Ki"), 5.0, Some(ResetType::ShortRest));
    ledger.register_resource(lr_resource.clone(), Some("1"), 4.0, Some(ResetType::LongRest));
...
fn test_variable_cost_logic() {
    ...
    if let ActionCost::Variable { resource_type: r, resource_val: val, min, max } = &cost {
        assert_eq!(r, &hp_pool);
        assert!(desired_amount >= *min);
        assert!(desired_amount <= *max);
        
        assert!(ledger.has(r.clone(), val.as_deref(), desired_amount));
        assert!(ledger.consume(r.clone(), val.as_deref(), desired_amount).is_ok());
    }
