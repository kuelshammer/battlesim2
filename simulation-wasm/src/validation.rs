...
pub fn check_single_requirement(
    ...
    match requirement {
        ...
        ActionRequirement::CombatState {
            condition: combat_condition,
        } => check_combat_condition(combat_condition, _context, combatant_id),
        ActionRequirement::StatusEffect {
            effect: effect_name,
        } => {
            let effects = _context.get_effects_on_target(combatant_id);
            effects.iter().any(|effect| {
                match &effect.effect_type {
                    crate::context::EffectType::Buff(buff) => {
                        buff.display_name.as_ref() == Some(effect_name)
                    }
                    crate::context::EffectType::Condition(condition) => {
                        format!("{:?}", condition) == *effect_name
                    }
                    _ => false, // Only buffs and conditions are considered for StatusEffect requirement
                }
            })
        }
        ActionRequirement::Custom {
